...
/* C89-compatible math wrappers (C99 float variants may not be available) */
#define nvg__sqrtf(x)    ((float)sqrt((double)(x)))
#define nvg__modf(x,y)   ((float)fmod((double)(x),(double)(y)))
#define nvg__sinf(x)     ((float)sin((double)(x)))
#define nvg__cosf(x)     ((float)cos((double)(x)))
#define nvg__tanf(x)     ((float)tan((double)(x)))
#define nvg__atan2f(y,x) ((float)atan2((double)(y),(double)(x)))
#define nvg__acosf(x)    ((float)acos((double)(x)))
#define nvg__ceilf(x)    ((float)ceil((double)(x)))
#define nvg__floorf(x)   ((float)floor((double)(x)))
...
NVGcolor nvgRGBA(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	...
	color.u.col.r = r / 255.0f;
	color.u.col.g = g / 255.0f;
	color.u.col.b = b / 255.0f;
	color.u.col.a = a / 255.0f;
	return color;
}
...
NVGcolor nvgLerpRGBA(NVGcolor c0, NVGcolor c1, float u)
{
	...
	for( i = 0; i <4; i++ )
	{
		cint.u.rgba[i] = c0.u.rgba[i] * oneminu + c1.u.rgba[i] * u;
	}
...
NVGcolor nvgHSLA(float h, float s, float l, unsigned char a)
{
	...
	m1 = 2 * l - m2;
	col.u.col.r = nvg__clampf(nvg__hue(h + 1.0f/3.0f, m1, m2), 0.0f, 1.0f);
	col.u.col.g = nvg__clampf(nvg__hue(h, m1, m2), 0.0f, 1.0f);
	col.u.col.b = nvg__clampf(nvg__hue(h - 1.0f/3.0f, m1, m2), 0.0f, 1.0f);
	col.u.col.a = a/255.0f;
	return col;
}
...
void nvgQuadTo(NVGcontext* ctx, float cx, float cy, float x, float y)
{
    ...
    float vals[7];
    vals[0] = NVG_BEZIERTO;
    vals[1] = x0 + 2.0f/3.0f*(cx - x0); vals[2] = y0 + 2.0f/3.0f*(cy - y0);
    vals[3] = x + 2.0f/3.0f*(cx - x); vals[4] = y + 2.0f/3.0f*(cy - y);
    vals[5] = x; vals[6] = y;
    nvg__appendCommands(ctx, vals, 7);
}
...
{
	float vals[2];
	vals[0] = NVG_WINDING; vals[1] = (float)dir;
...
void nvgRoundedRectVarying(NVGcontext* ctx, float x, float y, float w, float h, float radTopLeft, float radTopRight, float radBottomRight, float radBottomLeft)
{
	if(radTopLeft < 0.1f && radTopRight < 0.1f && radBottomRight < 0.1f && radBottomLeft < 0.1f) {
		...
		float vals[44];
		int vi = 0;
		vals[vi++] = NVG_MOVETO; vals[vi++] = x; vals[vi++] = y + ryTL;
		vals[vi++] = NVG_LINETO; vals[vi++] = x; vals[vi++] = y + h - ryBL;
		vals[vi++] = NVG_BEZIERTO; vals[vi++] = x; vals[vi++] = y + h - ryBL*(1 - NVG_KAPPA90); vals[vi++] = x + rxBL*(1 - NVG_KAPPA90); vals[vi++] = y + h; vals[vi++] = x + rxBL; vals[vi++] = y + h;
		vals[vi++] = NVG_LINETO; vals[vi++] = x + w - rxBR; vals[vi++] = y + h;
		vals[vi++] = NVG_BEZIERTO; vals[vi++] = x + w - rxBR*(1 - NVG_KAPPA90); vals[vi++] = y + h; vals[vi++] = x + w; vals[vi++] = y + h - ryBR*(1 - NVG_KAPPA90); vals[vi++] = x + w; vals[vi++] = y + h - ryBR;
		vals[vi++] = NVG_LINETO; vals[vi++] = x + w; vals[vi++] = y + ryTR;
		vals[vi++] = NVG_BEZIERTO; vals[vi++] = x + w; vals[vi++] = y + ryTR*(1 - NVG_KAPPA90); vals[vi++] = x + w - rxTR*(1 - NVG_KAPPA90); vals[vi++] = y; vals[vi++] = x + w - rxTR; vals[vi++] = y;
		vals[vi++] = NVG_LINETO; vals[vi++] = x + rxTL; vals[vi++] = y;
		vals[vi++] = NVG_BEZIERTO; vals[vi++] = x + rxTL*(1 - NVG_KAPPA90); vals[vi++] = y; vals[vi++] = x; vals[vi++] = y + ryTL*(1 - NVG_KAPPA90); vals[vi++] = x; vals[vi++] = y + ryTL;
		vals[vi++] = NVG_CLOSE;
		nvg__appendCommands(ctx, vals, vi);
	}
}
...
void nvgEllipse(NVGcontext* ctx, float cx, float cy, float rx, float ry)
{
	float vals[32];
	int vi = 0;
	vals[vi++] = NVG_MOVETO; vals[vi++] = cx-rx; vals[vi++] = cy;
	vals[vi++] = NVG_BEZIERTO; vals[vi++] = cx-rx; vals[vi++] = cy+ry*NVG_KAPPA90; vals[vi++] = cx-rx*NVG_KAPPA90; vals[vi++] = cy+ry; vals[vi++] = cx; vals[vi++] = cy+ry;
	vals[vi++] = NVG_BEZIERTO; vals[vi++] = cx+rx*NVG_KAPPA90; vals[vi++] = cy+ry; vals[vi++] = cx+rx; vals[vi++] = cy+ry*NVG_KAPPA90; vals[vi++] = cx+rx; vals[vi++] = cy;
	vals[vi++] = NVG_BEZIERTO; vals[vi++] = cx+rx; vals[vi++] = cy-ry*NVG_KAPPA90; vals[vi++] = cx+rx*NVG_KAPPA90; vals[vi++] = cy-ry; vals[vi++] = cx; vals[vi++] = cy-ry;
	vals[vi++] = NVG_BEZIERTO; vals[vi++] = cx-rx*NVG_KAPPA90; vals[vi++] = cy-ry; vals[vi++] = cx-rx; vals[vi++] = cy-ry*NVG_KAPPA90; vals[vi++] = cx-rx; vals[vi++] = cy;
	vals[vi++] = NVG_CLOSE;
	nvg__appendCommands(ctx, vals, vi);
...
void nvgFill(NVGcontext* ctx)
{
	...
	/* Apply global alpha */
	fillPaint.innerColor.u.col.a *= state->alpha;
	fillPaint.outerColor.u.col.a *= state->alpha;
...
void nvgStroke(NVGcontext* ctx)
{
	...
	if (strokeWidth < ctx->fringeWidth) {
		...
		strokePaint.innerColor.u.col.a *= alpha*alpha;
		strokePaint.outerColor.u.col.a *= alpha*alpha;
		strokeWidth = ctx->fringeWidth;
	}

	/* Apply global alpha */
	strokePaint.innerColor.u.col.a *= state->alpha;
	strokePaint.outerColor.u.col.a *= state->alpha;
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts)
{
	...
	/* Apply global alpha */
	paint.innerColor.u.col.a *= state->alpha;
	paint.outerColor.u.col.a *= state->alpha;
