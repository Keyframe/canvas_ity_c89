 ...
 *
 * C89 port for canvas_ity integration.
 */
...
/* Convenience macros for NVGcolor member access */
#define NVG_COLOR_R(c) ((c).u.col.r)
#define NVG_COLOR_G(c) ((c).u.col.g)
#define NVG_COLOR_B(c) ((c).u.col.b)
#define NVG_COLOR_A(c) ((c).u.col.a)
...
struct NVGtextRow {
	...
	const char* end;	/* Pointer to the input text where the row ends (one past last char). */
	const char* next;	/* Pointer to the beginning of the next row. */
	float width;		/* Logical width of the row. */
	float minx, maxx;	/* Actual bounds of the row. */
};
...
/* Sets the composite operation with custom pixel arithmetic for RGB and alpha separately. */
 ...
 * Render styles
 *
 * Fill and stroke render style can be either a solid color or a paint which is a
 * gradient or a pattern. Solid color is simply defined as a color value, different
 * kinds of paints can be created using nvgLinearGradient(), nvgBoxGradient(),
 * nvgRadialGradient() and nvgImagePattern().
 *
 ...
 * the shape. NanoVG uses even-odd fill rule to draw the shapes.
 */
...
/* Finds a loaded font of specified name, and returns handle to it, or -1 if not found. */
