...
/* nsvg__roundf is defined as a macro above */
...
static void nsvg__roundCap(NSVGrasterizer* r, NSVGpoint* left, NSVGpoint* right, NSVGpoint* p, float dx, float dy, float lineWidth, int ncap, int connect)
{
	...
	for (i = 0; i < ncap; i++) {
		...
		float ax = nsvg__cosf(a) * w, ay = nsvg__sinf(a) * w;
...
static void nsvg__roundJoin(NSVGrasterizer* r, NSVGpoint* left, NSVGpoint* right, NSVGpoint* p0, NSVGpoint* p1, float lineWidth, int ncap)
{
	...
	float dlx0 = p0->dy, dly0 = -p0->dx;
	float dlx1 = p1->dy, dly1 = -p1->dx;
	float a0 = nsvg__atan2f(dly0, dlx0);
	float a1 = nsvg__atan2f(dly1, dlx1);
	float da = a1 - a0;
	float lx, ly, rx, ry;
	...
	n = (int)nsvg__ceilf((nsvg__absf(da) / NSVG_PI) * (float)ncap);
	if (n < 2) n = 2;
	if (n > ncap) n = ncap;
...
{
	float da = nsvg__acosf(r / (r + tol)) * 2.0f;
	int divs = (int)nsvg__ceilf(arc / da);
	if (divs < 2) divs = 2;
	return divs;
}
...
static void nsvg__flattenShapeStroke(NSVGrasterizer* r, NSVGshape* shape, float scale)
{
	...
	for (path = shape->paths; path != NULL; path = path->next) {
		...
		if (shape->strokeDashCount > 0) {
			...
			dashOffset = nsvg__fmodf(shape->strokeDashOffset, allDashLen);
			if (dashOffset < 0.0f)
			...
			for (j = 1; j < r->npoints2; ) {
				...
				float dist = nsvg__sqrtf(dx*dx + dy*dy);
